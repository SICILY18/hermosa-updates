...
const Profile = () => {
    ...
    useEffect(() => {
        const initializeProfile = async () => {
            try {
                ...
                // Reset image state before fetching
                setImageError(false);
    ...
    const fetchProfileData = async () => {
        try {
            ...
            if (response.data) {
                ...
                // Only set preview image if there's a valid profile picture URL
                if (response.data.profile_picture) {
                    setPreviewImage(response.data.profile_picture);
                    setImageError(false); // Reset image error state
                } else {
                    setPreviewImage(null); // Reset to use default avatar
                    setImageError(false);
                }
            }
        } catch (error) {
    ...
    const getImageSource = () => {
        if (imageError || !previewImage) {
            const defaultAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(profileData.name || 'User')}&background=0D8ABC&color=fff&size=200`;
        ...
        const imageUrl = `${previewImage}${separator}t=${new Date().getTime()}`;
        console.log('Using image URL:', imageUrl);
        return imageUrl;
    };

    // Add effect to handle image refresh when preview changes
    useEffect(() => {
        if (previewImage && !previewImage.startsWith('blob:') && !previewImage.startsWith('https://ui-avatars.com')) {
